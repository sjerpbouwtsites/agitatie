...
{
  if (!IS_MEERTALIG) return $stringverzameling;
  $r = [];
  foreach ($stringverzameling as $s) {
    if ($s === 'language' || $s === 'post_translations') continue;
    $r[] = $s;
  }
  return $r;
}
...
{
  if (!IS_MEERTALIG) return $streng;
  if (!in_array($streng, $GLOBALS['agitatie_strengen'])) throw new \Exception("'" . $streng .  "' nog niet geregistreerd! Nog even doen in inc/taal.php", 1);
  return pll__($streng);
}
